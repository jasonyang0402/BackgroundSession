...
#import "SNLog.h"
#define SNLog(frmt,...) [SNLog logWithLine:__LINE__ method:[NSString stringWithFormat:@"%s", __FUNCTION__] time:[NSDate date] format:[NSString stringWithFormat:frmt, ## __VA_ARGS__]];
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(40, 49, 40, 40)];
...
- (void)application:(UIApplication *)application handleEventsForBackgroundURLSession:(NSString *)identifier completionHandler:(void (^)())completionHandler {
    ...
    SNLog(@"Rejoining session with identifier %@ %@", identifier, backgroundSession);
...
- (void)addCompletionHandler:(CompletionHandlerType)handler forSession:(NSString *)identifier {
    if ([self.completionHandlerDictionary objectForKey:identifier]) {
        SNLog(@"Error: Got multiple handlers for a single session identifier.  This should not happen.\n");
    }
...
- (void)callCompletionHandlerForSession:(NSString *)identifier {
    ...
    if (handler) {
        ...
        SNLog(@"Calling completion handler for session %@", identifier);
...
- (void)continueDownloadWithUrl:(NSString *)urlString; {
    if (!self.resumeData && urlString.length > 0) {
        ...
        SNLog(@"read resumedata from file");
    }
    if (self.resumeData) {
        if (IS_IOS10ORLATER) {
...
didFinishDownloadingToURL:(NSURL *)location {
    ...
    SNLog(@"downloadTask:%lu didFinishDownloadingToURL:%@", (unsigned long)downloadTask.taskIdentifier, location);
...
//    [delegate beginDownloadWithUrl:@"http://sw.bos.baidu.com/sw-search-sp/software/797b4439e2551/QQ_mac_5.0.2.dmg"];
    [delegate beginDownloadWithUrl:@"http://192.168.169.1/AKASO/NORM_20241212_160518.MOV"];
...
expectedTotalBytes:(int64_t)expectedTotalBytes {
    ...
    SNLog(@"fileOffset:%lld expectedTotalBytes:%lld",fileOffset,expectedTotalBytes);
}
...
totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite {
    ...
    if (self.lastRecevTime != 0 && (time >= 1000)) {
        ...
        SNLog(@"speed = %fM", speed);
        
        self.lastRecevTime = [[NSDate date] timeIntervalSince1970];
        self.lastReveDataLen = totalBytesWritten;
    }
    SNLog(@"downloadTask:%lu percent:%.2f%%",(unsigned long)downloadTask.taskIdentifier,(CGFloat)totalBytesWritten / totalBytesExpectedToWrite * 100);
...
- (void)URLSessionDidFinishEventsForBackgroundURLSession:(NSURLSession *)session {
    SNLog(@"Background URL session %@ finished events.\n", session);
...
//    SNLog(@"%s", __func__);
    SNLog(@"session identifier = %@, task url = %@", session.configuration.identifier, task.originalRequest.URL.absoluteString);
    if (error) {
        SNLog(@"error =%@", error);
        ...
        if ([error.userInfo objectForKey:NSURLSessionDownloadTaskResumeData]) {
            ...
            SNLog(@"resumeData = %@", [[NSString alloc] initWithData:resumeData encoding:NSUTF8StringEncoding]);
        }
    } else {
